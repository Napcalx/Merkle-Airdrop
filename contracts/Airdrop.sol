...
contract merkleAirdrop  is Modifiers, ERC20TokenReceiver {
    ...
    constructor(IERC20 _SkyToken, bytes32 _merkleRoot) {
        token = _Skytoken;
        merkleRoot = _merkleRoot;
    }

    error NotAddressZero();
    ...
    function claim (uint256 amount, bytes32 [] calldata proof ) external {
        if(msg.sender == address(0)) revert NotAddressZero();
        if(claimed[msg.sender]) revert AddressHasClaimed();
